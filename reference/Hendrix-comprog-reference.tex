...
\todo{Java \javadoc{lang}{Math} class}
...
can't necessarily assume that this will be available in a conteset
...
weight on each edge, Dijkstra's algorithm can find the shortest path
from a single chosen start vertex to every other vertex in the graph.
Figure~\ref{fig:dijkstra} has a basic implementation.
\begin{figure}
  \small
...
\begin{warning}
  ...
  ``decrease key'' method, on line 28 we have to instead do a \texttt{remove}
  followed by an \texttt{add}; but \texttt{remove} is $O(n)$, making
  the whole algorithm $O(VE)$.  If you really need $O(E \lg V)$
  performance, you can use an \fulllink{sec:adj-pq}.
\end{warning}
...
\item If the vertices of your graph are not naturally represented as
  ...
  the assignment.  Then run Dijkstra and translate the results to talk
  about the original vertices.
\item Dijkstra's algorithm uses addition to combine the weights of
  consecutive edges and \texttt{min} to pick the best path among
  parallel options.  However, there are other pairs of operations one
  can use with the same basic algorithm template.  For example, using
  \texttt{min}/\texttt{max} in place of \texttt{+}/\texttt{min} yields
  an algorithm which finds the path with the maximum possible minimum
  weight (\inlinekattis{vuk, crowdcontrol, muddyhike}).
\end{itemize}

\todo{Generalizing to other semirings e.g. max/min.}
...
\kattis{balanceddiet, drivinglanes}
