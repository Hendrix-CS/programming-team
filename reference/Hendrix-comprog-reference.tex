...
\begin{figure}
  \small
  \code{graph/DGraph.java}
  \code{graph/DEdge.java}
  \caption{Dijkstra's algorithm} \label{fig:dijkstra}
\end{figure}

% TODO: clean this up, remove DGraph stuff, just the algorithm itself
...
\begin{itemize}
\item \textbf{Ceiling division} (\inlinekattis{soylent,wordcloud,amultiplicationgame}).
  If \texttt{p} and \texttt{q} are of type \texttt{int}, then
  \texttt{p/q} computes $\lfloor p/q \rfloor$, the quotient (rounded
  down).  If you want the quotient rounded \emph{up}, that is,
  $\lceil p/q \rceil$, compute \[ \texttt{(p + q - 1) / q}. \]
