...
\todo{Examples of using \texttt{PriorityQueue}.
  \texttt{Collections.reverseOrder()} for max PQ.  Show how to
  construct with custom \texttt{Comparator}, eg. using lambda
  notation; things like \texttt{Double.compare}, etc.} \todo{Note lack
  of decreaseKey operation (Dijkstra), use remove + add, not as fast}
...
\todo{hashset and treeset-based bag implementations}
...
\item The parent map is also optional, and can be used to reconstruct
  an actual shortest path from $s$ to any vertex (see below).
...
\todo{Show how to reconstruct a shortest path using parent map}
