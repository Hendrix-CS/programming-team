...
multiples of $p$ in the table; we know a number is prime if when we
get to it it has not been crossed out.  This takes time
...
instead of an array of \texttt{boolean} which uses less memory.
Constructing a \texttt{PrimeSieve} of size $10^8$ should take about a second
and use only about 12 MB of memory; constructing smaller prime sieves
should be quite fast.

\code{math/PrimeSieve.java}

\todo{Sieving (factors, Euler totient?).}
...
\kattis{fenwick, supercomputer, turbo, moviecollection}
...
\item Exact Set Cover with Algorithm X/dancing links
\item Matrix powers
...
\item Faster primality testing with Miller-Rabin (\eg testing with $a
  ...
  deterministic) or Baille-PSW which is known to be deterministic up
  to $2^{64}$.
\end{itemize}
