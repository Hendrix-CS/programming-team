...
% that $f(n)$ is equal to some target value---except that $f$ is defined
% on the \emph{integers} instead of the real numbers.
...
\item \textbf{Derangements} (\inlinekattis{secretsanta}).  The number
  ...
  original place is
  \[ !n = n \cdot !(n-1) + (-1)^n = n! \sum_{i=0}^n \frac{(-1)^i}{i!}
    = \left[ \frac{n!}{e} \right], \]
