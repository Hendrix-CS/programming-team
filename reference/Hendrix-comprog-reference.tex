...
An \term{Eulerian path} is one which traverses every edge exactly once
(but may visit vertices multiple times).  An \term{Eulerian circuit}
is an Eulerian path which starts and ends at the same vertex.

Checking whether an Eulerian path/circuit \emph{exists} is simple:
...
To \emph{find} an Eulerian path, use Hierholzer's
Algorithm. \todo{Explain, example code.}
...
other than the source or sink.  Flow networks can be used to model a
...
\section{Longest Common Subsequence (LCS) via LIS}
...
\todo{Basic principles of combinatorics.  mod $10^9 + 7$. Code for
  computing binomial coefficients. Multinomial coefficients. Large
  binomial coefficients modulo a prime (modular inverse factorial
  tables, Lucas's theorem).}
...
\item Convolutions with Fast Fourier Transform / Number Theoretic
  Transform (\inlinekattis{tiles, aplusb, kinversions})
\end{itemize}
