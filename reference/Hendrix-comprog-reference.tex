...
\kattis{increasingsubsequence,longincsubseq,manhattanmornings}

A \emph{subsequence} of a sequence is a subset of elements, taken in
...
Conceptually, to compute the LIS of a sequence, we build a
\emph{downravel}, a set of nonincreasing subsequences which partition
the original sequence.  We keep these subsequences as a list of stacks, and
maintain the invariant that their top elements are always
sorted from smallest to biggest.  We iterate through the elements of
$S$ in order, and push each onto the leftmost possible stack whose top
is $\geq$ the element being added.
...
Naively, this runs in $O(n^2)$ time, since for each of the the $n$
...
What if we want not just the \emph{length} of the LIS but the LIS
itself?
