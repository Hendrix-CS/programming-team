...
and combine them to produce a new value.  Given indices $i$ and $j$,
...
$\max$, that is, we want to find the maximum value in a range
...
\item For this to make sense, the combining operation must typically
  be \emph{associative} ($a \oplus (b \oplus c) = (a \oplus b) \oplus
  c$). \todo{Mention word ``semigroup''.}
\item Sometimes there is also an inverse operation $\ominus$ which
  ``cancels out'' the effects of the combining operation.  For
  example, subtraction cancels out addition. On the other hand, there
  is no operation that can cancel out the effect of taking a
  maximum. \todo{Mention word ``group''; link to more info?}
...
another \emph{prefix scan array} $P[0 \dots n]$ such that $P[i]$
